Connections::CourseTeacherConnection = Types::CourseUserType.define_connection do
  ...
  description 'Возвращает объекты CourseTeacher вместе с total_count.'
  ...
  field :total_count do
    ...
    resolve ->(_, _, ctx) {
      ...
      course_teachers = course_teachers.where(user_id: parent_args_hash['user_id']).order('is_current DESC, continued_at DESC') if parent_args_hash.key? 'user_id'
      course_teachers = course_teachers.joins(:course).where("courses.name ILIKE '%#{parent_args_hash['q']}%'").order('courses.name DESC') if parent_args_hash.key? 'q'
